...
fn cleanup_temp_files() {
    ...
    if temp_file.exists() {
        if let Err(e) = unlink(&temp_file) {
            eprintln!("Warning: Failed to clean up temp file {}: {}", temp_file.display(), e);
        }
    }
}
...
fn build_dosr_binary(pid: u32, temp_file: &PathBuf) -> Result<(), Box<dyn Error>> {
    let user = User::from_name(&std::env::var("RAR_USER").or_else(|_| std::env::var("SUDO_USER")).expect("RAR_USER not set"))
        .unwrap_or(None)
        .ok_or("User not found")?;
    let user_name_cstr =
        CString::new(user.name.clone()).inspect_err(|e| eprintln!("Failed to create CString: {}",e))?;
    ...
    env::set_var("PATH", format!("{}:{}/bin", env::var("PATH")?, env!("CARGO_HOME")));
    ...
    let output = Command::new("cargo")
        ...
        .env("RAR_CFG_PATH", cfg_path.to_str().ok_or("Invalid RAR_CFG_PATH")?)
