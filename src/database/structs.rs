...
impl PartialEq<str> for SActorType {
    fn eq(&self, other: &str) -> bool {
        match self {
            ...
            SActorType::Id(id) => other.parse().and_then( | oid : u32 | Ok(oid == *id)).unwrap_or(false),
        }
    }
}
...
impl Into<Vec<Group>> for SGroups {
    fn into(self) -> Vec<Group> {
        match self {
            ...
            SGroups::Multiple(groups) => groups.into_iter().map(| x | x.into_group().unwrap().unwrap()).collect(),
        }
    }
}
...
impl SActor {
    pub fn from_user_string(user: &str) -> Self {
        SActor::User { id: Some(user.into()), _extra_fields: Map::default() }
    }
    pub fn from_user_id(id: u32) -> Self {
        SActor::User { id: Some(id.into()), _extra_fields: Map::default() }
    }
    pub fn from_group_string(group: &str) -> Self {
        SActor::Group { groups: Some(SGroups::Single(group.into())), _extra_fields: Map::default() }
    }
...
mod tests {
    ...
    fn test_unknown_fields() {
        ...
        assert_eq!(config.roles[0].as_ref().borrow()[0].as_ref().borrow()._extra_fields.get("unknown").as_ref().unwrap().as_str().unwrap(), "unknown");
        let role = config.roles[0].as_ref().borrow();
        let cred = &role[0].as_ref().borrow().cred;
        assert_eq!(cred._extra_fields.get("unknown").unwrap(), "unknown");
        let capabilities = cred.capabilities.as_ref().unwrap();
        assert_eq!(capabilities._extra_fields.get("unknown").unwrap(), "unknown");
        let commands = &as_borrow!(role[0]).commands;
    ...
    fn test_sgroups_compare() {
        ...
        let multiple = SGroups::Multiple(vec![SActorType::Name("single".into()), SActorType::Id(1000)]);
        assert!(single == single);
        assert!(single <= multiple);
        assert!(multiple >= single);
        assert!(multiple == multiple);
        let multiple2 = SGroups::Multiple(vec![SActorType::Name("single".into()), SActorType::Id(1001)]);
