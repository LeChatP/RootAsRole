...
pub fn get_capability_description(cap : &Cap) -> &'static str {
 ...
 This capability was added in Linux 5.9 to separate out checkpoint/restore functionality from
 the overloaded CAP_SYS_ADMIN capability."#,
        Cap::CHOWN => r#"Make arbitrary changes to file UIDs and GIDs (see chown(2))."#,
        Cap::DAC_OVERRIDE => r#"Bypass file read, write, and execute permission checks. (DAC is an abbreviation of
 ...
 descriptor."#,
        Cap::FOWNER => r#"• Bypass permission checks on operations that normally require the filesystem UID of the
 process to match the UID of the file (e.g., chmod(2), utime(2)), excluding those operations
 ...
 • Allocate memory using huge pages (memfd_create(2), mmap(2), shmctl(2))."#,
        ...
        Cap::KILL => r#"Bypass permission checks for sending signals (see kill(2)). This includes use of the ioctl(2)
 ...
 Allow MAC configuration or state changes. Implemented for the Smack Linux Security Module
 (LSM)."#,
 ...
 • use setsockopt(2) to set the following socket options: SO_DEBUG, SO_MARK, SO_PRIORITY (for
 a priority outside the range 0 to 6), SO_RCVBUFFORCE, and SO_SNDBUFFORCE."#,
 ...
 This capability was added in Linux 5.8 to separate out performance monitoring functionality
 from the overloaded CAP_SYS_ADMIN capability. See also the kernel source file
 ...
 user_namespaces(7) for details."#,
        Cap::SETPCAP => r#"If file capabilities are supported (i.e., since Linux 2.6.24): add any capability from the
 calling thread's bounding set to its inheritable set; drop capabilities from the bounding set
 ...
 property of CAP_SETPCAP is not available when the kernel is configured to support file
 capabilities, since CAP_SETPCAP has entirely different semantics for such kernels.)"#,
        Cap::SETUID => r#"• Make arbitrary manipulations of process UIDs (setuid(2), setreuid(2), setresuid(2),
 setfsuid(2));
 ...
 • employ CLONE_* flags that create new namespaces with clone(2) and unshare(2) (but, since
 Linux 3.8, creating user namespaces does not require any capability);
 ...
 • employ the TIOCSTI ioctl(2) to insert characters into the input queue of a terminal other
 than the caller's controlling terminal;
 ...
 • before Linux 2.6.25: drop capabilities from the system-wide capability bounding set."#,
        Cap::SYS_NICE => r#"• Lower the process nice value (nice(2), setpriority(2)) and change the nice value for
 arbitrary processes;
 ...
 /proc/sys/kernel/kptr_restrict has the value 1. (See the discussion of the kptr_restrict
 in proc(5).)"#,
