...
pub fn get_capability_description(cap : &Cap) -> &'static str {
 ...
 descriptor."#,
        Cap::FOWNER => r#"• Bypass permission checks on operations that normally require the filesystem UID of
 ...
 • set the set-group-ID bit for a file whose GID does not match the filesystem or any of
 ...
 • Allocate memory using huge pages (memfd_create(2), mmap(2), shmctl(2))."#,
        ...
        Cap::KILL => r#"Bypass permission checks for sending signals (see kill(2)). This includes use of the
 ...
 functionality from the overloaded CAP_SYS_ADMIN capability. See also the kernel source
 ...
 namespace; see user_namespaces(7) for details."#,
        Cap::SETPCAP => r#"If file capabilities are supported (i.e., since Linux 2.6.24): add any capability from
 ...
 umount(2), pivot_root(2), swapon(2), swapoff(2), sethostname(2), and
 setdomainname(2);
 ...
 CAP_CHECKPOINT_RESTORE (but the latter, weaker capability is preferred for accessing
 that functionality).
 ...
 with CAP_SYS_RESOURCE."#,
        Cap::SYS_TIME => r#"Set system clock (settimeofday(2), stime(2), adjtimex(2)); set real-time (hardware)
 clock."#,
