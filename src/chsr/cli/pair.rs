...
fn match_pair(pair: &Pair<Rule>, inputs: &mut Inputs) -> Result<(), Box<dyn Error>> {
    match pair.as_rule() {
        ...
        Rule::to => {
            ...
            convertion.to_type = inner.next().unwrap().as_str().to_lowercase().parse().inspect_err(|&e| {
                warn!(
        ...
        Rule::from => {
            ...
            convertion.from_type =
                Some(inner.next().expect("from_type not found").as_str().to_lowercase().parse().inspect_err(|&e| {
                    warn!(
                        "Unknown type {}, types available : {}",
                        e,
                        StorageMethod::VARIANTS.join(", ")
