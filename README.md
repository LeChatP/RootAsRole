...
One can remove the bit s from Linux executables and use our module to run them securely. However, this will not work for all of them because some of these commands are coded to check the uid 0 and not the privileges. This is the case of passwd. In fact, passwd checks whether the real uid is 0 or not. When the real uid is 0 (i.e. process run by root user) the programs allows the root user to change the passwords of any user. When the real uid is not 0, then the user can change only his own password (for more information check the source code of passwd and especially the variable amroot https://github.com/shadow-maint/shadow/blob/master/src/passwd.c). Passwd program must be thus modified to allow any user who has the convenient privilege to modify the passwords of other users. For example, an administrator may give the privilege cap_dac_override to user awazan only to modify the passwords other users by limiting the use of this privilege to passwd program.

Lets remove the bit s from ping because ping is privilege-aware program. Interestingly, ping's code source has a function called limit_capabilites() (https://github.com/iputils/iputils/blob/master/ping.c). It uses this function to remove the list of root privileges that it doesn't need ( ping needs only cap_net_raw capability). However, not all programs are coded correctly with regards the use of root privileges. Administrators can use our module to secure the execution of programs regardless of the quality of their code.
...
7-Test our module on other dirtributions of Linux and make our installation et configuration scripts applicable to them.
